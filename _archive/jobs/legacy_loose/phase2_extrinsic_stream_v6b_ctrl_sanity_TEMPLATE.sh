#!/bin/bash
# v6b CTRL (α=1) sanity eval — 5 PBS jobs, one per latency, N=50.
# Same as v6b sanity but points at sft_multilingual_v6b_ctrl/final.
# Purpose: compare α=1 vs α=5 on BLEU/AL/chunks-per-sent.
...
for LAT in "${LATENCIES[@]}"; do
    ...
    PBS_FILE="${REPO}/jobs/phase2_extrinsic_stream_v6b_ctrl_sanity_${LAT_TAG}.pbs"
    LOG_FILE="${REPO}/logs/phase2_extrinsic_stream_v6b_ctrl_sanity_${LAT_TAG}.log"

    cat > "$PBS_FILE" <<EOF
#!/bin/bash
#PBS -N v6bC_san_${LAT_TAG}
#PBS -P po67
#PBS -q gpuhopper
#PBS -l ncpus=12
#PBS -l ngpus=1
...
for D in "\${DIRS[@]}"; do
    ...
    echo ""
    echo "===== v6b-ctrl | check_argmax | latency='\$LAT' | \$D | \$N ====="
