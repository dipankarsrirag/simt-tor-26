#!/bin/bash
# Wrapper: generate + qsub 5 latency PBS files for v6b-merged sanity eval.
# Args:   $1 = model dir suffix (merged / merged3 / merged5)
# Ex:     bash phase2_extrinsic_stream_v6b_merged_sanity_TEMPLATE.sh merged
...
for LAT in "${LATENCIES[@]}"; do
    ...
    PBS_FILE="${REPO}/jobs/phase2_extrinsic_stream_v6b_${VARIANT}_sanity_${LAT_TAG}.pbs"
    LOG_FILE="${REPO}/logs/phase2_extrinsic_stream_v6b_${VARIANT}_sanity_${LAT_TAG}.log"

    cat > "$PBS_FILE" <<EOF
#!/bin/bash
#PBS -N v6b${VARIANT}_${LAT_TAG}
#PBS -P po67
#PBS -q gpuhopper
#PBS -l ncpus=12
#PBS -l ngpus=1
...
MODEL=results/phase2/sft_multilingual_v6b_ctrl_${VARIANT}/final
...
for D in "\${DIRS[@]}"; do
    ...
    OUT_FILE="\$OUT/flores_stream_v6b${VARIANT}_checkargmax_${LAT_TAG}_\${D}_n${N}.json"
    if [ -f "\$OUT_FILE" ]; then echo "SKIP: \$OUT_FILE"; continue; fi
    echo "===== v6b-${VARIANT} | latency='\$LAT' | \$D | N=\$N ====="
    python -u src/eval/extrinsic.py \\
        --model_dir "\$MODEL" \\
        --tokenizer_dir results/phase2/tokenizer-extended-v6 \\
        --dev_src "\$SRC_FILE" --dev_ref "\$TGT_FILE" \\
        --n_sentences \$N \\
        --src_lang "\$SRC" --tgt_lang "\$TGT" \\
        --use_chat_template --latency "\$LAT" \\
