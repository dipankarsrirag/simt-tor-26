...
Stage II is a stretch — see `_archive/docs/TIMELINE.md`. Do not spend Phase 2 budget on it.
...
| B (ours) | Backbone, per `_archive/docs/method-formal.md`. Same wrapping and indicators. |
...
- **Stretch — WMT22 X↔En, 8 directions (De/Zh/Ru/Cs ↔ En).** Matches EAST Figure 4. Only touch after Gate 3 (`_archive/docs/TIMELINE.md`) passes and only if Stage II is being attempted — sentence-level SiMT numbers here require a Stage-II-trained model. Zh↔En and Cs↔En are where the reordering-divergence claim lives (fronted objects, verb-final subordinate clauses); those are the interesting directions if we run this at all.
