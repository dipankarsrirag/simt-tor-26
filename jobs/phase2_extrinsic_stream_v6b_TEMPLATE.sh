#!/bin/bash
# Generate + submit 5 v6b eval PBS jobs — one per latency string.
# Each PBS runs all 8 directions (drops zh) × check_argmax × N=1012 sents FLORES-200.
# Model: results/phase2/sft_multilingual_v6b/final (best-model-by-eval-loss).
#
# Latencies (5-point NL ladder — 3 base training + 2 inference-only interpolation):
#   low, low-medium, medium, medium-high, high
#
# Usage:  bash jobs/phase2_extrinsic_stream_v6b_TEMPLATE.sh
#         (call AFTER v6b SFT finishes and `results/phase2/sft_multilingual_v6b/final/` exists)

set -eo pipefail
REPO=/g/data/ba39/dipankar/simt-tor-26
LATENCIES=("low" "low-medium" "medium" "medium-high" "high")
N=1012   # full FLORES devtest

for LAT in "${LATENCIES[@]}"; do
    LAT_TAG=$(echo "$LAT" | tr '-' '_')
    PBS_FILE="${REPO}/jobs/phase2_extrinsic_stream_v6b_${LAT_TAG}.pbs"
    LOG_FILE="${REPO}/logs/phase2_extrinsic_stream_v6b_${LAT_TAG}.log"

    cat > "$PBS_FILE" <<EOF
#!/bin/bash
#PBS -N v6b_eval_${LAT_TAG}
#PBS -P po67
#PBS -q gpuhopper
#PBS -l ncpus=12
#PBS -l ngpus=1
...
# v6b eval — latency=${LAT}, 8 directions × ${N} sents FLORES devtest.
...
MODEL=results/phase2/sft_multilingual_v6b/final
...
for D in "\${DIRS[@]}"; do
    ...
    OUT_FILE="\$OUT/flores_stream_v6b_checkargmax_${LAT_TAG}_\${D}_n${N}.json"
    if [ -f "\$OUT_FILE" ]; then echo "SKIP: \$OUT_FILE"; continue; fi
    echo ""
    echo "===== v6b | check_argmax | latency='\$LAT' | \$D | \$N sents FLORES ====="
...
echo ""
echo "PBS files generated (NOT submitted). To submit after v6b SFT completes:"
echo "  for f in ${REPO}/jobs/phase2_extrinsic_stream_v6b_*.pbs; do qsub \$f; done"
