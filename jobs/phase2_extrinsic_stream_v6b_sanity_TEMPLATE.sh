#!/bin/bash
# v6b SANITY eval — 5 PBS jobs, one per latency, N=50 sentences per direction.
# 8 directions (drops zh) × check_argmax. ~5-8 min per PBS on H200.
# Purpose: get a sense of the v6b BLEU signal quickly before committing to
# the full N=1012 FLORES devtest evaluation.
#
# Latencies: low, low-medium, medium, medium-high, high (5-point NL ladder)
#
# Usage:  bash jobs/phase2_extrinsic_stream_v6b_sanity_TEMPLATE.sh
...
for LAT in "${LATENCIES[@]}"; do
    ...
    PBS_FILE="${REPO}/jobs/phase2_extrinsic_stream_v6b_sanity_${LAT_TAG}.pbs"
    LOG_FILE="${REPO}/logs/phase2_extrinsic_stream_v6b_sanity_${LAT_TAG}.log"

    cat > "$PBS_FILE" <<EOF
#!/bin/bash
#PBS -N v6b_san_${LAT_TAG}
#PBS -P po67
#PBS -q gpuhopper
#PBS -l ncpus=12
#PBS -l ngpus=1
...
# v6b sanity eval — latency=${LAT}, 8 directions × ${N} sents FLORES devtest.
# check_argmax, chat-template prompt.
