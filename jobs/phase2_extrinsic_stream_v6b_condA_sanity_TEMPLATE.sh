#!/bin/bash
# Cond-A (GPT-4 chunks) sanity eval — matches v6b-ctrl sanity setup exactly:
# same FLORES devtest, same first N=50 sentences, same 5 latencies.
# ONLY 4 directions (de-en, en-de, ru-en, en-ru) since cond-A didn't train
# on ar/vi (Multi-90K doesn't ship those).
#
# Direct comparison target: v6b-ctrl (α=1, our OT chunks) on same 4 dirs.
...
for LAT in "${LATENCIES[@]}"; do
    ...
    PBS_FILE="${REPO}/jobs/phase2_extrinsic_stream_v6b_condA_sanity_${LAT_TAG}.pbs"
    LOG_FILE="${REPO}/logs/phase2_extrinsic_stream_v6b_condA_sanity_${LAT_TAG}.log"

    cat > "$PBS_FILE" <<EOF
#!/bin/bash
#PBS -N v6bA_san_${LAT_TAG}
#PBS -P po67
#PBS -q gpuhopper
#PBS -l ncpus=12
#PBS -l ngpus=1
...
for D in "\${DIRS[@]}"; do
    ...
    OUT_FILE="\$OUT/flores_stream_v6bcondA_checkargmax_${LAT_TAG}_\${D}_n${N}.json"
...
echo ""
echo "PBS files generated (NOT submitted). Fire after cond-A SFT finishes:"
echo "  for f in ${REPO}/jobs/phase2_extrinsic_stream_v6b_condA_sanity_*.pbs; do qsub \$f; done"
