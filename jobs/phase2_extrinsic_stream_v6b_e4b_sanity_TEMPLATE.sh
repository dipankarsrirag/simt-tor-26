#!/bin/bash
# v6b-CTRL-E4B sanity eval — matches v6b-ctrl sanity setup: same FLORES
# devtest first N=50, same 8 dirs, same 5 latencies. Only model_dir differs.
...
for LAT in "${LATENCIES[@]}"; do
    ...
    PBS_FILE="${REPO}/jobs/phase2_extrinsic_stream_v6b_e4b_sanity_${LAT_TAG}.pbs"
    LOG_FILE="${REPO}/logs/phase2_extrinsic_stream_v6b_e4b_sanity_${LAT_TAG}.log"

    cat > "$PBS_FILE" <<EOF
#!/bin/bash
#PBS -N v6bE4_san_${LAT_TAG}
#PBS -P po67
#PBS -q gpuhopper
#PBS -l ncpus=12
#PBS -l ngpus=1
...
MODEL=results/phase2/sft_multilingual_v6b_ctrl_e4b/final
...
for D in "\${DIRS[@]}"; do
    ...
    OUT_FILE="\$OUT/flores_stream_v6be4b_checkargmax_${LAT_TAG}_\${D}_n${N}.json"
    if [ -f "\$OUT_FILE" ]; then echo "SKIP: \$OUT_FILE"; continue; fi
    echo ""
    echo "===== v6b-E4B | latency='\$LAT' | \$D | N=\$N ====="
