...
#
# Usage: nohup bash jobs/resubmit_missing_evals.sh > logs/resubmit_missing_evals.log 2>&1 &
...
while true; do
    ...
    while IFS= read -r pbs; do
        ...
        if [ -n "$out" ] && [ -f "/g/data/ba39/dipankar/simt-tor-26/results/phase2/extrinsic/$out" ]; then
            SUBMITTED[$pbs]=DONE
            continue
        fi
